...
import com.euler.system.mapper.SysConfigMapper;
...
public class SysConfigServiceImpl implements ISysConfigService {
    ...
    public TableDataInfo<SysConfig> selectPageConfigList(SysConfig config, PageQuery pageQuery) {
        ...
        LambdaQueryWrapper<SysConfig> lqw = new LambdaQueryWrapper<SysConfig>()
            .likeRight(StringUtils.isNotBlank(config.getConfigName()), SysConfig::getConfigName, config.getConfigName())
            .eq(StringUtils.isNotBlank(config.getConfigType()), SysConfig::getConfigType, config.getConfigType())
            .eq(StringUtils.isNotBlank(config.getStatus()), SysConfig::getStatus, config.getStatus())
            .eq(config.getOperationPlatform() != null, SysConfig::getOperationPlatform, config.getOperationPlatform())
            .eq(config.getGameId() != null, SysConfig::getGameId, config.getGameId())

            .likeRight(StringUtils.isNotBlank(config.getConfigKey()), SysConfig::getConfigKey, config.getConfigKey())
    ...
    public List<SysConfig> selectConfigList(SysConfig config) {
        ...
        if (config.getDevice()!=null)
            devices.add(config.getDevice());

        List<Integer> platforms=new ArrayList<>();
        platforms.add(-1);
        if (config.getOperationPlatform()!=null)
            platforms.add(config.getOperationPlatform());
        ...
        LambdaQueryWrapper<SysConfig> lqw = new LambdaQueryWrapper<SysConfig>()
            ...
            .eq(config.getOperationPlatform() != null, SysConfig::getOperationPlatform, config.getOperationPlatform())
            .eq(config.getGameId() != null, SysConfig::getGameId, config.getGameId())
            .eq(config.getDevice() != null, SysConfig::getDevice, config.getDevice())
     ...
     */
    @Override
    public List<SysConfig> selectUserConfigList(SysConfig config) {
        Map<String, Object> params = config.getParams();
        List<Integer> devices=new ArrayList<>();
        devices.add(-1);
        if (config.getDevice()!=null)
        ...
        LambdaQueryWrapper<SysConfig> lqw = new LambdaQueryWrapper<SysConfig>()
            .eq( SysConfig::getStatus, UserConstants.NORMAL)
            .in(config.getOperationPlatform() != null, SysConfig::getOperationPlatform, platforms)
            .eq(config.getGameId() >0, SysConfig::getGameId, config.getGameId())
            .in(config.getDevice() != null, SysConfig::getDevice, devices);
        return baseMapper.selectList(lqw);
    }
