...
public class SysProfileController extends BaseController {
    ...
    public R<Void> updateProfile(@RequestBody SysUser user) {
        if (StringUtils.isNotEmpty(user.getPhonenumber())
            ...
            return R.fail("修改用户'" + user.getUserName() + "'失败，手机号码已存在");
                ...
                && UserConstants.NOT_UNIQUE.equals(userService.checkEmailUnique(user.getEmail(),LoginHelper.getUserId()))) {
    ...
    public R<Map<String, Object>> avatar(@RequestPart("avatarfile") MultipartFile file) throws IOException {
        if (!file.isEmpty()) {
            SysFile sysFile = remoteFileService.upload(file.getName(), file.getOriginalFilename(), file.getContentType(), LoginHelper.getUserType().getUserType(),"upyun", file.getBytes());
            if (ObjectUtil.isNull(sysFile)) {
                return R.fail("文件服务异常，请联系客服");
            }
